...
function createChannel(streamIn) {
    ...
    let buildOrServe = (args)=>{
        ...
        let logPluginError = (e, pluginName, note, done)=>{
            let flags = [];
            try {
                pushLogFlags(flags, options, {
                }, isTTY, buildLogLevelDefault);
            } catch (e2) {
            }
        ...
        if (plugins && plugins.length > 0) {
            ...
            handlePlugins(options, plugins, key, details).then((result)=>{
                if (!result.ok) {
                    ...
                    try {
                        buildOrServeContinue(__spreadProps(__spreadValues({
                        }, args), {
                            key,
                            details,
                            logPluginError,
    ...
    let buildOrServeContinue = ({ callName , refs: callerRefs , serveOptions , options , isTTY , defaultWD , callback , key , details , logPluginError , requestPlugins , runOnEndCallbacks , pluginRefs  })=>{
        ...
        let { entries , flags , write , stdinContents , stdinResolveDir , absWorkingDir , incremental , nodePaths , watch  } = flagsForBuildOptions(callName, options, isTTY, buildLogLevelDefault, writeDefault);
        let request = {
            command: "build",
            key,
            entries,
    ...
    let transform2 = ({ callName , refs , input , options , isTTY , fs , callback  })=>{
        ...
        let start = (inputPath)=>{
            try {
                ...
                try {
                    pushLogFlags(flags, options, {
                    }, isTTY, transformLogLevelDefault);
                } catch (e2) {
                }
